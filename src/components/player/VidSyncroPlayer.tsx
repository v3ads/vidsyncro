...
export default function VidSyncroPlayer({
  ...
  // ── Autoplay: both start muted (browser policy). User taps unmute once.
  useEffect(() => {
    if (!isLoaded) return
    const a = videoARef.current
    const b = videoBRef.current
  ...
  useEffect(() => {
    ...
    const onWaitingA = onWaiting('a')
    const onWaitingB = onWaiting('b')
    const onPlayingA = onPlaying('a')
    const onPlayingB = onPlaying('b')

    a.addEventListener('waiting', onWaitingA)
    b.addEventListener('waiting', onWaitingB)
    a.addEventListener('playing', onPlayingA)
    b.addEventListener('playing', onPlayingB)

    return () => {
      a.removeEventListener('waiting', onWaitingA)
      b.removeEventListener('waiting', onWaitingB)
      a.removeEventListener('playing', onPlayingA)
      b.removeEventListener('playing', onPlayingB)
      if (bufferTimeout) clearTimeout(bufferTimeout)
    }
  }, [isLoaded])

  // ── Unmute: one tap. Audio is then managed by switchToA/switchToB. ─────────
  // Always unmutes the currently visible video and mutes the hidden one.
  // From this point on, switching A↔B carries sound to whichever is visible.
  const handleUnmute = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    ...
    const b = videoBRef.current
    if (!a || !b) return
    // Unmute whichever video is currently visible, keep other muted
    a.muted = showingB
    b.muted = !showingB
    setIsMuted(false)
  }, [showingB])
  ...
  // ── Switch: audio follows the visible video when unmuted ──────────────────
  ...
  return (
    ...
    >
      ...
      <video
        ...
        loop={ec.loop}
        preload="auto"
        poster={getMuxThumbUrl(videoA!.muxPlaybackId!, 0)}
      />
      ...
      <video
        ...
        loop={ec.loop}
        preload="auto"
        poster={getMuxThumbUrl(videoB!.muxPlaybackId!, 0)}
      />
      ...
      {/* ── UNMUTE BUTTON ─────────────────────────────────────────────────────
          Bottom-left corner — away from the hint badge (bottom-center) and the
          Reality B badge (top-left). Icon-only so one thumb can tap it while
          the other hand holds the screen. Disappears after one tap.
          Never shown when holding to reveal B — the user needs both hands.   */}
      {isLoaded && isPlaying && isMuted && !ec.muted && !showingB && (
        <button
          ...
          style={{
            ...
            boxShadow: '0 0 18px rgba(124,58,237,0.7), 0 2px 8px rgba(0,0,0,0.5)',
      ...
      {oc.showSwitchIndicator && switchFlash && (
        <div
          ...
          style={{ boxShadow: `inset 0 0 0 3px ${oc.indicatorColor || '#8b5cf6'}` }}
        />
      )}
        ...
        <div className="absolute inset-0 z-10 flex flex-col justify-end pointer-events-none">
          {!isPlaying && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-auto">
              <button
                onClick={e => { e.stopPropagation(); playBoth() }}
...
export default function VidSyncroPlayer({ project, onAnalyticsEvent, preview = false }: VidSyncroPlayerProps) {
  ...
  // When showing B: screen is completely clean — no buttons, no badges, no overlays.
  // The only thing visible is Video B filling the screen.
  ...
  // Unmute button appears on Video A only — user taps once, sound follows from then on.
  useEffect(() => {
    if (!isLoaded) return
    const a = videoARef.current
    const b = videoBRef.current
  ...
  // One-tap unmute. After this, switchToA/B carry sound to whichever video is visible.
  const handleUnmute = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation(); e.preventDefault()
    const a = videoARef.current; const b = videoBRef.current
    if (!a || !b) return
    // At unmute time we are always on A (button hidden during B)
  ...
  return (
    ...
    >
      ...
      {/* Buffering overlay — shown on both A and B since buffering affects both */}
      {isLoaded && isBuffering && (
      ...
      {/* ── EVERYTHING BELOW IS VIDEO A ONLY (showingA) ────────────────────── */}

      {/* Unmute button — bottom-left, icon only, disappears after one tap */}
